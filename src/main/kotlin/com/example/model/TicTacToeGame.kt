...
class TicTacToeGame {
    ...
    fun connectPlayer(session: WebSocketSession): Char? {
        ...
        //If 'X' is connected, assigns 'O' to the new player; otherwise, assigns 'X'.
    ...
    fun finishedTurn(player: Char, x: Int, y: Int) {
        //Checks if the cell is already occupied or if the game has already been won.
        ...
        state.update {
            //Updates the game field with the player's move.
            val newField = it.field.also { field ->
                field[y][x] = currentPlayer
            }
            // Checks if the board is full.
            val isBoardFull = newField.all { it.all { it != null } }
            //If the board is full, schedules a delay before starting a new round.
            if (isBoardFull) {
